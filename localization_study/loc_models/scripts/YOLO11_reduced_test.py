...
if __name__ == "__main__":
    ...
    BASE_PATH = os.path.join(project_root, "loc_datasets", "reduced_data")
    ...
    for class_name in CLASS_LIST:
        for reduction_factor in REDUCTION_LEVEL:
            ...
            run_number = "01"
            run_name = f"25conf_reduced_{reduction_factor}_{class_name}_test_run_{run_number}"      # We test with confidence threshold 0.25
            project_name = f"yolo11-test-reduced-{reduction_factor}-labels"
            ...
            # Test saved model
            model_path = os.path.join(f"{CHECKPOINT_PATH}/yolo11-train-reduced-{reduction_factor}-labels/{reduction_factor}_{class_name}_train_run_{run_number}/weights/best.pt")
